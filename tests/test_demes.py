...
class TestHypothesisStrategies:
    ...
    @hyp.given(tests.epochs_lists())
    def test_epochs_lists(self, e):
        # e is a list of Epoch
        pass

    @hyp.settings(suppress_health_check=[hyp.HealthCheck.too_slow])
    @hyp.given(tests.graphs())
    def test_graphs(self, g):
        # g is a Graph
        g.validate()

    @hyp.given(tests.deme_ids())
    def test_deme_ids(self, id):
        # id is a valid deme ID
        id.isidentifier()
